...
class CompilerHost {
  constructor(
    ...
    this.createProgram = this.typescript.createEmitAndSemanticDiagnosticsBuilderProgram;
  ...
  readFile(path, encoding) {
    ...
    if (content && VueProgram_1.VueProgram.isVue(path)) {
      const resolved = VueProgram_1.VueProgram.resolveScriptBlock(
        this.typescript,
        content
      );
  ...
  writeFile(_path, _data, _writeByteOrderMark) {
    if (this.enableEmitFiles) {
      fs_extra_1.outputFileSync(_path, _data);
    }
  }
