...
class ForkTsCheckerWebpackPlugin {
  constructor(options) {
    this.startAt = 0;
    this.computeContextPath = filePath =>
  ...
  apply(compiler) {
    ...
    const tslintOk =
      !this.tslintPath || FsHelper_1.FsHelper.existsSync(this.tslintPath);
    ...
    if ('hooks' in this.compiler) {
      ...
      this.compiler.plugin('compile', () => {
        ...
        this.compiler.applyPluginsAsync(
          hooks_1.legacyHookMap.serviceBeforeStart,
          () => {
            if (this.cancellationToken) {
              // request cancellation if there is not finished job
              this.cancellationToken.requestCancellation();
              this.compiler.applyPlugins(
                hooks_1.legacyHookMap.cancel,
            ...
            try {
              ...
              this.compiler.applyPlugins(
                hooks_1.legacyHookMap.serviceStartError,
                error
              );
            }
          }
  ...
  spawnService() {
      ...
      [],
      {
        ...
        env: Object.assign({}, process.env, {
          ...
          COMPILER_OPTIONS: JSON.stringify(
            Object.assign({}, this.compilerOptions, { noEmit: this.noEmit })
          ),
    ...
    if ('hooks' in this.compiler) {
      ...
      // webpack 2 / 3
      this.compiler.applyPlugins(
        hooks_1.legacyHookMap.serviceStart,
        this.tsconfigPath,
        this.tslintPath,
        this.watchPaths,
        this.workersNumber,
  ...
  handleServiceMessage(message) {
    ...
    this.diagnostics = message.diagnostics.map(
      NormalizedMessage_1.NormalizedMessage.createFromJSON
    );
    this.lints = message.lints.map(
      NormalizedMessage_1.NormalizedMessage.createFromJSON
    );
    if (this.ignoreDiagnostics.length) {
      this.diagnostics = this.diagnostics.filter(
        diagnostic =>
          !this.ignoreDiagnostics.includes(parseInt(diagnostic.code, 10))
      );
    }
    if (this.ignoreLints.length) {
      this.lints = this.lints.filter(
        lint => !this.ignoreLints.includes(lint.code)
      );
    }
...
ForkTsCheckerWebpackPlugin.ALL_CPUS =
  os.cpus && os.cpus() ? os.cpus().length : 1;
ForkTsCheckerWebpackPlugin.ONE_CPU_FREE = Math.max(
  1,
  ForkTsCheckerWebpackPlugin.ALL_CPUS - 1
);
ForkTsCheckerWebpackPlugin.TWO_CPUS_FREE = Math.max(
  1,
