...
import { wrapRpc } from './wrap';

const WORKER_DATA_ENV_KEY = 'WORKER_DATA';

interface RpcWorker<T extends RpcMethod = RpcMethod> {
  connect(): void;
  terminate(): void;
  call: RpcRemoteMethod<T>;
...
function createRpcWorker<T extends RpcMethod>(
  ...
  const worker: RpcWorker<T> = {
    ...
    get call() {
      ...
      if (!remoteMethod) {
        return (() =>
          Promise.reject('Worker is not connected - cannot perform RPC.')) as RpcRemoteMethod<T>;
      }
      return remoteMethod;
    },
    get connected() {
