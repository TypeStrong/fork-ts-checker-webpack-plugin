...
class IncrementalChecker {
  ...
  nextIteration() {
    if (!this.watcher) {
      const watchExtensions = this.vue
        ? ['.ts', '.tsx', '.vue']
        : ['.ts', '.tsx'];
      this.watcher = new FilesWatcher_1.FilesWatcher(
        this.watchPaths,
        watchExtensions
      );
  ...
  loadVueProgram() {
    this.programConfig =
      this.programConfig ||
      VueProgram_1.VueProgram.loadProgramConfig(
        this.typescript,
        this.programConfigFile,
        this.compilerOptions
      );
    return VueProgram_1.VueProgram.createProgram(
      this.typescript,
      this.programConfig,
  ...
  getDiagnostics(cancellationToken) {
    ...
    const workSet = new WorkSet_1.WorkSet(
      filesToCheck,
      this.workNumber,
      this.workDivision
    );
    ...
    return Promise.resolve(
      NormalizedMessage_1.NormalizedMessage.deduplicate(
  ...
  getLints(cancellationToken) {
    ...
    workSet.forEach(fileName => {
      ...
      try {
        ...
        if (
          FsHelper_1.FsHelper.existsSync(fileName) &&
  ...
  emitFiles() {
    ...
    console.log(
      'nao achei o writefile usado no program.emit() inside IncrementalChecker'
    );
    console.log('------------------------------');
  }
}
